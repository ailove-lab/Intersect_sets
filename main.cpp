...
// Необходимо проверить процент актинвости пользователей из deep-activity в опросе
using namespace std;
...
int main(int argc, char** argv) {
    ...
    if(!get_allfiles(argv[1], files_A) || !get_allfiles(argv[2], files_B)) return 1;
        ...
        if(file.is_open()) {
            ...
            while(getline(file, uid)) {
                ...
                for(auto&& s: folder_A_sets) {
                    ...
                    if(s.second.find(id) != s.second.end()) intersections[name]++;
                }
                c++;
            }
            cout << f << "\t" << c << endl;
